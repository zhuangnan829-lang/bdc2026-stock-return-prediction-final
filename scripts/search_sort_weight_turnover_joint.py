...
DEFAULT_OUTPUT_DIR = "app/model/sort_weight_turnover_joint_search"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Joint search for sort strategy, weighting scheme, and max turnover.")
    ...
    parser.add_argument("--top_n_report", type=int, default=24)
...
def build_grid() -> list[dict]:
    ...
    sort_strategies = ["pure_prediction", "risk_adjusted"]
    weighting_schemes = ["equal", "pred", "risk_adjusted"]
    max_turnovers = [0.50, 0.55, 0.60, 0.65, 0.70, 0.75, 1.00]

    grid = []
    for sort_strategy, weighting_scheme, max_turnover in itertools.product(
        sort_strategies, weighting_schemes, max_turnovers
    ):
        grid.append(
            {
                **base,
                "profile_name": (
                    f"swt_{sort_strategy}_{weighting_scheme}_mt{int(max_turnover * 100):03d}"
                ),
...
def is_current_default(profile: dict) -> bool:
    return (
        profile["sort_strategy"] == BEST_CONFIG["selection"]["sort_strategy"]
        and profile["weighting_scheme"] == BEST_CONFIG["selection"]["weighting_scheme"]
...
def main() -> None:
    ...
    summary_path = output_dir / "sort_weight_turnover_joint_summary.csv"
    top_path = output_dir / "sort_weight_turnover_joint_top.csv"
    ...
    best_candidate = {
        ...
        "turnover_ratio_upper_pct": float(BEST_CONFIG["risk_filter_thresholds"]["turnover_ratio_upper_pct"]),
        "risk_penalty_weight": float(BEST_CONFIG["risk_filter_thresholds"]["risk_penalty_weight"]),
        "sort_strategy": str(best["sort_strategy"]),
    ...
    best_candidate_path = output_dir / "sort_weight_turnover_best_candidate.json"
    best_candidate_path.write_text(json.dumps(best_candidate, ensure_ascii=False, indent=2), encoding="utf-8")

    report_lines = [
        "# Sort Weight Turnover Joint Search",
        "",
        ...
        f"- sort_strategy: `{best['sort_strategy']}`",
        f"- weighting_scheme: `{best['weighting_scheme']}`",
        f"- max_turnover: `{float(best['max_turnover']):.2f}`",
    ...
    if not current_default_row.empty:
        ...
        report_lines.extend(
            [
                ...
                f"- sort_strategy: `{current['sort_strategy']}`",
                f"- weighting_scheme: `{current['weighting_scheme']}`",
                f"- max_turnover: `{float(current['max_turnover']):.2f}`",
    ...
    report_lines.extend(
        [
            "## Top Candidates",
            "",
            "| rank | profile_name | sort_strategy | weighting_scheme | max_turnover | cum_after_cost | sharpe_after_cost | max_drawdown_after_cost | avg_turnover |",
            "|---:|---|---|---|---:|---:|---:|---:|---:|",
        ]
    )
    for _, row in result_df.head(args.top_n_report).iterrows():
        report_lines.append(
            f"| {int(row['rank_after_cost'])} | {row['profile_name']} | {row['sort_strategy']} | "
            f"{row['weighting_scheme']} | {float(row['max_turnover']):.2f} | "
    ...
    report_path = output_dir / "sort_weight_turnover_joint_report.md"
    report_path.write_text("\n".join(report_lines), encoding="utf-8")

    print(result_df.head(args.top_n_report).to_string(index=False))
    print(f"[sort_weight_turnover_joint] wrote {summary_path}")
    print(f"[sort_weight_turnover_joint] wrote {top_path}")
    print(f"[sort_weight_turnover_joint] wrote {best_candidate_path}")
    print(f"[sort_weight_turnover_joint] wrote {report_path}")
