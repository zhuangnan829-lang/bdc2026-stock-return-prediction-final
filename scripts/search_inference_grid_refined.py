...
from config import BEST_PROFILE_NAME, BEST_CONFIG
...
DEFAULT_OUTPUT_DIR = "app/model/inference_grid_search_refined"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Refined local search around the current best inference profile.")
    ...
    parser.add_argument("--top_n_report", type=int, default=15)
    return parser.parse_args()


def build_refined_grid() -> list[dict]:
    base = {
        "top_k": BEST_CONFIG["selection"]["top_k"],
        "enable_risk_filters": 1,
        "allow_cash_fallback": 0,
        "turnover_rate_lower_pct": BEST_CONFIG["risk_filter_thresholds"]["turnover_rate_lower_pct"],
        "turnover_rate_upper_pct": BEST_CONFIG["risk_filter_thresholds"]["turnover_rate_upper_pct"],
        "turnover_ratio_upper_pct": BEST_CONFIG["risk_filter_thresholds"]["turnover_ratio_upper_pct"],
    ...
    candidate_sizes = [40, 45, 50]
    vol20_pcts = [0.86, 0.88, 0.90]
    vol5_pcts = [0.88, 0.90, 0.92]
    risk_penalty_weights = [0.18, 0.20, 0.22]
    max_turnovers = [0.50, 0.55, 0.60]
    turnover_ratio_high_pcts = [0.93, 0.95, 0.97]

    grid = []
    for candidate_size, vol20, vol5, rp, mt, trh in itertools.product(
        candidate_sizes,
        vol20_pcts,
        vol5_pcts,
        ...
        grid.append(
            {
                **base,
                "profile_name": (
                    f"refined_cs{candidate_size}_v20{int(vol20*100)}_v5{int(vol5*100)}_"
                    f"tr{int(trh*100)}_rp{int(rp*100)}_mt{int(mt*100)}"
                ),
                "primary_candidate_size": candidate_size,
                "max_volatility_20d_pct": vol20,
                "max_volatility_5d_pct": vol5,
                "turnover_ratio_upper_pct": trh,
...
def main() -> None:
    ...
    summary_path = output_dir / "inference_grid_search_refined_summary.csv"
    top_path = output_dir / "inference_grid_search_refined_top.csv"
    ...
    report_lines = [
        "# Refined Inference Grid Search Report",
        "",
        ...
        "## Top Candidates",
        "",
        "| rank | profile_name | candidate_size | vol20_pct | vol5_pct | turnover_ratio_high | risk_penalty | max_turnover | cum_after_cost | sharpe_after_cost | avg_turnover |",
        "|---:|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for _, row in result_df.head(args.top_n_report).iterrows():
        report_lines.append(
            ...
            f"{float(row['max_volatility_5d_pct']):.2f} | {float(row['turnover_ratio_upper_pct']):.2f} | "
    ...
    report_path = output_dir / "inference_grid_search_refined_report.md"
    report_path.write_text("\n".join(report_lines), encoding="utf-8")

    print(result_df.head(args.top_n_report).to_string(index=False))
    print(f"[refined_grid_search] wrote {summary_path}")
    print(f"[refined_grid_search] wrote {top_path}")
    print(f"[refined_grid_search] wrote {report_path}")
