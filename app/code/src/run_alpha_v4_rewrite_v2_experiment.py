...
OUTPUT_DIR = MODEL_DIR / "alpha_v4_rewrite_v2_experiment"
...
EXPERIMENTS = [
    ...
    },
    {
        ...
        "skip_train": False,
        "notes": "更收敛的相对强弱加速度 v2。",
    },
    {
        "label": "trend_persist_v2",
        "feature_set": "base_alpha_v3_rs_crowding_mini4__trend_persistence_score_10d_v2",
        "model_dir": MODELS_DIR / "trend_persist_v2",
        "result_path": RESULTS_DIR / "trend_persist_v2.csv",
        "backtest_summary_path": MODELS_DIR / "trend_persist_v2" / "backtest" / "backtest_summary.csv",
        "skip_train": False,
        "notes": "更温和的趋势持续性 v2。",
    },
    {
        "label": "rs_accel_v2__trend_persist_v2",
        "feature_set": "base_alpha_v3_rs_crowding_mini4__rel_strength_accel_5d_v2__trend_persistence_score_10d_v2",
        "model_dir": MODELS_DIR / "rs_accel_v2__trend_persist_v2",
        "result_path": RESULTS_DIR / "rs_accel_v2__trend_persist_v2.csv",
        "backtest_summary_path": MODELS_DIR / "rs_accel_v2__trend_persist_v2" / "backtest" / "backtest_summary.csv",
        "skip_train": False,
        "notes": "rs_accel v2 与 trend_persist v2 联合版本。",
    },
]
...
def write_report(summary_df: pd.DataFrame, report_path: Path) -> None:
    baseline = summary_df[summary_df["label"] == "current_mini4_default"].iloc[0]
    candidates = summary_df[summary_df["label"] != "current_mini4_default"].copy()
    best_slice = candidates.sort_values(
        ["delta_case_slice_score", "delta_cumulative_return_after_cost"],
        ascending=[False, False],
    ).iloc[0]
    best_defensive = candidates.sort_values(
        ["delta_cumulative_return_after_cost", "delta_case_slice_score"],
        ascending=[False, False],
    ).iloc[0]
    ...
    lines = [
        ...
        "## 结论",
        "",
        f"- 切片分数提升最明显的是 `{best_slice['label']}`，提升 `{best_slice['delta_case_slice_score']:.6f}`。",
        f"- 对本地累计收益伤害最小的是 `{best_defensive['label']}`，变化 `{best_defensive['delta_cumulative_return_after_cost']:.6f}`。",
...
def main() -> None:
    ...
    baseline_row = collect_row(EXPERIMENTS[0], baseline_row=None)
    rows = [baseline_row]
    for exp in EXPERIMENTS[1:]:
        rows.append(collect_row(exp, baseline_row=baseline_row))

    summary_df = pd.DataFrame(rows)
    summary_path = OUTPUT_DIR / "alpha_v4_rewrite_v2_summary.csv"
    retained_path = OUTPUT_DIR / "alpha_v4_rewrite_v2_retained.csv"
    report_path = OUTPUT_DIR / "alpha_v4_rewrite_v2_report.md"
    ...
    print(f"[alpha_v4_rewrite_v2] wrote {summary_path}")
    print(f"[alpha_v4_rewrite_v2] wrote {retained_path}")
    print(f"[alpha_v4_rewrite_v2] wrote {report_path}")
